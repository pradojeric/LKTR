...
<div class="row p-3 bg-primary mb-0 jumbotron justify-content-center">
    <div class="col-md-12">
        <h3 class="text-center text-white mb-0">Add User</h3>
    </div>
</div>
<form method="post" action="{{ route('teachers.store') }}">
    ...
    <div class="row mt-0 jumbotron bg-light justify-content-center">
            ...
            <div class="col-sm-6 form-group">
                ...
                <select class="form-control @error('course_id') is-invalid @enderror" name="role_id" id="role_id">
                    <option selected hidden disabled>...</option>
                    @foreach($roles as $role)
                        <option value="{{ $role->id }}" @if($role->id == old('role_id')) selected @endif>{{ $role->role }}</option>
                    @endforeach
                </select>
                @error('course_id')
                    <span class="invalid-feedback" role="alert">
